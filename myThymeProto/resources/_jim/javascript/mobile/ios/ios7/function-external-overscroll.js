...
//		}
	...
	drag = function (event) {
		...
		if($target) {
			...
			else if(event.data.options.direction === 'multi' && $target.closest(".firer").is(".swipeleftdown, .swipeleftup, .swiperightdown, .swiperightup, .pinchopen, .pinchclose, .rotateleft, .rotateright")) {
				stop(event);
				return;
			}
		}
...
//			target.css('cursor', compat.cursorGrabbing);
	...
	stop = function (event) {
		...
		//calculate final displacement of container and assign to scroll
		var children = target.children().not('.verticalScroll').not('.horizontalScroll');
		if(children.size()>=1) {
			var pos = jQuery(children[0]).position(),
			zoom = 1/jimUtil.getTotalScale();
			target.children(".horizontalScroll").attr("desplX", pos.left*zoom);
			target.children(".verticalScroll").attr("desplY", pos.top*zoom);
		}
